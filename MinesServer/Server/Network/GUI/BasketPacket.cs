...
{
    public readonly record struct BasketPacket(long g, long r, long b, long v, long w, long c, int capacity) : IDataPart<BasketPacket>
    {
        ...
        public static BasketPacket Decode(ReadOnlySpan<byte> decodeFrom)
        {
            ...
            return new(long.Parse(parts[0]), long.Parse(parts[1]), long.Parse(parts[2]), long.Parse(parts[3]), long.Parse(parts[4]), long.Parse(parts[5]), int.Parse(parts[6]));
        }
